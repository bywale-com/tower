...
export default async function IntelligencePage() {
  ...
  const [{ data: topic }, { data: space }] = await Promise.all([
    supabase.from("topics").select("*").eq("slug", "canada-immigration").single(),
    supabase.from("spaces").select("id, title").eq("slug", "canada-immigration").single(),
  ]);
  ...
  // Normalise bar chart — max bar height = 100%
  ...
  // Normalise popular queries for interest-share bar
          ...
          <div className="flex items-start justify-between mb-4">
            <div>
              <h2 className="text-5xl font-black font-headline text-on-surface tracking-tight mb-2">
                {topic?.title ?? "Canada Immigration"}
              </h2>
        ...
        <div className="col-span-12 md:col-span-4 bg-surface-container-low rounded-xl p-6">
          ...
          {popularQueries && popularQueries.length > 0 ? (
            <div className="space-y-5">
              {popularQueries.map(
                (
                  item: { query_text: string; rate_per_hour: number | null },
                  i: number
                ) => (
                  <div
                    key={i}
                  ...
                  >
                    ...
                    <span className="text-[10px] font-mono text-on-surface-variant">
                      {item.rate_per_hour != null
                        ? `${item.rate_per_hour.toFixed(0)} Q/HR`
                        : "—"}
                    </span>
                  </div>
                )
        ...
        <div className="col-span-12 md:col-span-4 bg-surface-container-low rounded-xl p-6 overflow-hidden">
          ...
          <div className="space-y-6 mt-6">
            {creators && creators.length > 0 ? (
              ...
              <p className="text-xs text-outline font-mono text-center py-4">
          ...
          {popularQueries && popularQueries.length > 0 ? (
            <table className="w-full text-left">
              ...
              <tbody className="divide-y divide-white/5">
                {popularQueries.map(
                  (
                    q: { query_text: string; rate_per_hour: number | null },
                    i: number
                  ) => (
                    <tr
                      key={i}
                      ...
                      <td className="px-6 py-4">
                        <div className="w-full h-1 bg-white/5 rounded-full">
                          <div
                            ...
                            style={{
                              width: `${((q.rate_per_hour ?? 0) / topQueriesMax) * 100}%`,
                            }}
                          />
                        </div>
                      </td>
          ...
          {risingQueries && risingQueries.length > 0 ? (
            <table className="w-full text-left">
              ...
              <tbody className="divide-y divide-white/5">
                {risingQueries.map(
                  (
                    q: { query_text: string; growth_pct: number | null; is_breakout: boolean | null },
                    i: number
                  ) => (
                    <tr
                      key={i}
            ...
            <div className="flex items-center justify-between mb-10">
              <div>
                <h3 className="text-xl font-headline font-bold">
                  Answer Efficiency Distribution
                </h3>
                <p className="text-sm text-on-surface-variant">
                  Monthly breakdown of signal fulfillment
                </p>
              </div>
                  ...
                  return (
                    ...
                    >
                      <div className="w-full flex justify-center gap-1 items-end h-full">
                        <div
                          className="w-1/3 bg-primary-container rounded-t-sm"
                          style={{ height: `${ansPct}%` }}
                        />
                        <div
                          className="w-1/3 bg-surface-container-highest rounded-t-sm"
                          style={{ height: `${unansPct}%` }}
                        />
                      </div>
                      <span
                        className={`text-[10px] font-mono mt-2 opacity-50 ${
                          isCurrent ? "text-primary font-bold opacity-100" : ""
                        }`}
                      >
      ...
      {tags && tags.length > 0 && (
        <section>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">
            ...
            <div
              className="bg-surface-container-low p-4 rounded-lg flex items-center justify-center opacity-40 hover:opacity-100 transition-opacity cursor-pointer"
              style={{ border: "1px dashed rgba(255,255,255,0.05)" }}
            >
              <span className="material-symbols-outlined text-sm">add</span>
              <span className="text-[10px] font-mono ml-2">EXPLORE ALL</span>
            </div>
          </div>
        </section>
      )}
